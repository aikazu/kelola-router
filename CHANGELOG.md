...
- **QwenCloud upstream provider.** Eighth upstream: Aliyun Model Studio **token-plan** (`credit_type='token-plan'`), a single **native Anthropic-Messages** endpoint at `https://token-plan.ap-southeast-1.maas.aliyuncs.com/apps/anthropic/v1/messages` with Bearer `sk-sp-` key auth — no OAuth, no migration. Routed via `qctp/` prefix. The transform forces `stream:true` upstream, so non-stream clients are served by re-aggregating the native Anthropic SSE (`aggregateAnthropicSSE` → `message` JSON; OpenAI-format non-stream via `responseAnthropicToOpenAI`). **OpenAI streaming is rejected with 501** (no Anthropic-SSE → OpenAI-SSE converter). Aliyun error envelope `{request_id, code, message}`: `InvalidApiKey` → 401, `InvalidParameter` → 400. Builtin 3-model catalogue (1M input / 128K output, per-M pricing $2/$6, $0.44/$1.32, $1.32/$3.96) in `src/providers/qwencloud/models.ts`; dashboard card + CLI runner (`npm run add-account -- --provider qwencloud`) and `npm run seed-qwencloud-models` (also in `seed-all` / `seed-all:docker`).
...
- **Repo restructure (layout + naming).** `src/auth.ts` folded into `src/auth/index.ts` (kills the file/dir name collision); the request-augment module moved from the src root into the proxy layer (`src/proxy/augment.ts`); MiniMax provider folderized (`src/providers/minimax/`, parity with the other six providers) absorbing the single-file `runtime/` dir. Filenames normalized to kebab-case everywhere: `client_keys`→`client-keys`, `requestLogs`→`request-logs`, `quotaSnapshots`→`quota-snapshots`, `seedBuiltinModels`→`seed-builtin-models`, plus ~30 camelCase modules and ~50 test files (dot/camelCase separators → hyphen). Auth/account-scoped client components moved under `components/accounts/`; client tests consolidated in `src/__tests__/`. Dead `Placeholder` page and tracked `graphify-out/` scratch removed. Docs (AGENTS/ARCHITECTURE/MEMORY/README, guides, reference, `.claude/docs`) synced to the new layout. No behavior change.

### Verification

- Server **1004 tests** (164 files), client **121 tests** — server suite runs on the `forks` pool by default (`vitest.config.ts`); the `threads` pool segfaults at teardown with better-sqlite3 on Windows/Node 24 (pre-existing, unrelated to this release).
...
- `request-logs.ts` insert signature gains an optional `req_id` field; existing call sites stay unchanged (field is nullable, no migration required for new rows).
...
- Native Anthropic streaming for `/v1/messages` (Claude Code, hermes-agent) — `message_start` → `content_block_*` (text / thinking / tool_use) → `message_delta` → `message_stop`. `/v1/chat/completions` streams OpenAI SSE. Both pipe through `pipe-with-usage` for telemetry.
