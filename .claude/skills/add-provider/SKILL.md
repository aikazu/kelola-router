...
3. **Migration (if needed)** — `src/db/migrations/00X-<name>.ts`: additive `ALTER TABLE ADD COLUMN` only. Register in `src/db/migrations/index.ts` `ALL_MIGRATIONS`. Bump ID past current `user_version` (currently 10, next = 11).
