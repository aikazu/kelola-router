...
1. **Pick next ID** — `git grep user_version` or read `src/db/migrations/index.ts`. Current = 5, next = 6. File: `src/db/migrations/00X-<kebab-name>.ts` (or `00N-…` matching the ID).
...
4. **Upgrade-path test** — `src/db/migrations/00X-<name>.test.ts`:
   - Pattern A: fresh DB → run `migrate(db)` → assert column/table exists, `user_version` = N.
   - Pattern B: DB at `user_version = N-1` → run `migrate(db)` → assert it applies, `user_version` = N.
