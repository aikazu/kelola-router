...
export function Overview() {
  ...
  return (
    <>
      <TopBar title={<>Over<em>view</em></>} eyebrow="Operations / 7-day window" />
      ...
      <div class="ov-hero">
        <div class="ov-hero-figure surface">
          ...
          {isLoading || !data ? (
            ...
            <>
              <div class="hero-figure">$<em>{data.stats.totalCost.toFixed(2)}</em></div>
              <div class="hero-figure-sub">{data.stats.totalRequests.toLocaleString()} requests · {data.stats.totalTokens.toLocaleString()} tokens</div>
            </>
          )}
        </div>
        <div class="ov-hero-meta surface">
          ...
          {isLoading || !data ? (
            ...
            <div class="specsheet" style={{ marginTop: 12 }}>
              <div class="specsheet-row"><span class="specsheet-label">Upstream</span><span class="specsheet-value">{data.stats.enabledAccounts} / {data.stats.totalAccounts} enabled</span></div>
              <div class="specsheet-row"><span class="specsheet-label">Client keys</span><span class="specsheet-value">{data.stats.activeClientKeys} active</span></div>
              <div class="specsheet-row"><span class="specsheet-label">Models live</span><span class="specsheet-value">{data.byModel.length}</span></div>
            </div>
          )}
        </div>
      </div>
        ...
        <Card title="By model" eyebrow="Last 7 days">
          {isLoading || !data ? <TableSkeleton rows={3} cols={3} /> :
           data.byModel.length === 0 ? <p class="card-sub">No requests yet.</p> : (
            <table class="tbl">
              <thead><tr><th>Model</th><th class="num">Cost</th><th class="num">Requests</th></tr></thead>
              <tbody>{data.byModel.map(m => <tr key={m.model}><td>{m.model}</td><td class="num mono">${m.cost.toFixed(4)}</td><td class="num mono">{m.requests.toLocaleString()}</td></tr>)}</tbody>
            </table>
          )}
        </Card>

        <Card title="Recent requests" eyebrow="Live stream" actions={<a href="#/admin/usage" class="btn btn-ghost btn-sm">View all →</a>}>
          {isLoading || !data ? <TableSkeleton rows={5} cols={5} /> :
           data.recent.length === 0 ? <p class="card-sub">No traffic yet.</p> : (
            <table class="tbl">
              <thead><tr><th>Time</th><th>Model</th><th>Status</th><th class="num">Latency</th><th class="num">Cost</th></tr></thead>
              <tbody>{data.recent.map(r => (
                <tr key={r.id}
                  ...
                  <td title={r.createdAt} class="mono">{relativeTime(r.createdAt)}</td>
                  <td>{r.model}</td>
