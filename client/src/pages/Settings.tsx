...
interface SettingsData { caveman: { level: string }; caching: { autoBreakpoints: boolean }; rtk: { enabled: boolean }; minimax: { upstreamFormat?: string }; }
...
export function Settings() {
  ...
  const cavemanMut = useMutation({ mutationFn: (level: string) => apiFetch("/api/admin/settings/caveman", { method: "POST", json: { level } }), onSuccess: () => { qc.invalidateQueries({ queryKey: ["settings"] }); toast.success("Saved"); } });
  const rtkMut = useMutation({ mutationFn: (enabled: boolean) => apiFetch("/api/admin/settings/rtk", { method: "POST", json: { enabled } }), onSuccess: () => { qc.invalidateQueries({ queryKey: ["settings"] }); toast.success("Saved"); } });
  const cachingMut = useMutation({ mutationFn: (autoBreakpoints: boolean) => apiFetch("/api/admin/settings/caching", { method: "POST", json: { autoBreakpoints } }), onSuccess: () => { qc.invalidateQueries({ queryKey: ["settings"] }); toast.success("Saved"); } });
  const minimaxMut = useMutation({ mutationFn: (b: object) => apiFetch("/api/admin/settings/minimax", { method: "POST", json: b }), onSuccess: () => { qc.invalidateQueries({ queryKey: ["settings"] }); toast.success("Saved"); } });
  const pwMut = useMutation({ mutationFn: (b: { action: string; password?: string }) => apiFetch("/api/admin/settings/password", { method: "POST", json: b }), onSuccess: () => { qc.invalidateQueries({ queryKey: ["me"] }); toast.success("Updated"); } });

  if (isLoading || !data) return <><TopBar title="Settings" /><p style={{ color: "var(--text-3)" }}>Loading…</p></>;
