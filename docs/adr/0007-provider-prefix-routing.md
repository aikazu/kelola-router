...
Use a regex or prefix list on the model name itself (e.g. `claude-` → Kiro, `MiniMax-` → MiniMax). Rejected because: brittle, breaks the moment a provider changes their naming convention, and requires the router to know provider-specific name patterns, which is coupling that doesn't belong in the routing layer.
