...
| `rtk` | `{ enabled: boolean }` | `{ enabled: true }` | Runtime filter compression (RTK) toggle | `POST /api/admin/settings/rtk` |
| `caching` | `{ autoBreakpoints: boolean }` | `{ autoBreakpoints: true }` | Inject `cache_control` breakpoints on system + last user message | `POST /api/admin/settings/caching` |
...
| `selection.codebuddy` | same shape as `selection.minimax` | same | Account selection strategy for CodeBuddy accounts | same |
| `selection.pioneer` | same shape as `selection.minimax` | same | Account selection strategy for Pioneer accounts | same |
...
| `build.version` | string | (none) | Build version string surfaced on the dashboard sidebar (read-only, set by build pipeline from package.json) | Set by build pipeline |
