...
**Symptom:** `cd client && npm run lint` (→ `biome check .`) reports
...
— excluding the entire client subtree.
...
— all **pre-existing** formatter + a11y noise across the whole tree
...
- **AddModelModal / EditModelModal** pricing inputs — side-by-side `<label style={{flex:1}}>`
...
- **TransportAssignment** — 0 eligible: 3 selects, 1 checkbox, 1 number input with `min={1}`
  + clamping logic (`Field` has no `min`/`max`/`step`).

`Field` is **text-input only** by design (`Field.tsx:18-21`). Extending it to selects/
textareas/`min`/`aria-required` is a deliberate future decision, not leftover debt.
