...
Model prefix
: Clients send `pio/<pioneer-model-id>`, verbatim. This includes ids that carry their own vendor prefix, e.g. `pio/claude-opus-4-8` or `pio/moonshotai/Kimi-K2.6`. `parseModelPrefix` splits only on the first slash, so everything after `pio/` is the full Pioneer id.
