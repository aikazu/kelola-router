...
describe("proxy uses parseError to surface windowResetMs", () => {
  ...
  it("applies windowResetMs cooldown when upstream returns 2056 with model_remains", async () => {
    ...
    const acc = createAccount(db, { id: "acc_pe", label: "a1", credit_type: "payg", api_key: "mm_test" });
    ...
    const row = db.prepare(`SELECT rate_limited_until, backoff_level FROM accounts WHERE id = ?`).get(acc.id) as { rate_limited_until: string | null; backoff_level: number };
  ...
  it("applies retryAfterSec cooldown when Retry-After header present", async () => {
    ...
    const acc = createAccount(db, { id: "acc_ra", label: "a1", credit_type: "payg", api_key: "mm_test" });
    upsertModel(db, { name: "MiniMax-M2.7", upstream_model: "MiniMax-M2.7" });
    ...
    const row = db.prepare(`SELECT rate_limited_until FROM accounts WHERE id = ?`).get(acc.id) as { rate_limited_until: string | null };
    expect(row.rate_limited_until).toBeTruthy();
    const untilMs = new Date(row.rate_limited_until!).getTime();
