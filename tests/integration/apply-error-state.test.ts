...
import { updateAccount } from "../../src/db/repos/accounts.js";
...
describe("proxy uses applyErrorState centrally", () => {
  ...
  it("resets account backoff to 0 on 200 success", async () => {
    ...
    const acc = createAccount(db, { id: "acc_aes", label: "a1", credit_type: "payg", api_key: "mm_test" });
    ...
    const row = db.prepare(`SELECT backoff_level, rate_limited_until FROM accounts WHERE id = ?`).get(acc.id) as { backoff_level: number; rate_limited_until: string | null };
  ...
  it("bumps backoff and sets rate_limited_until on 429 baseResp 1002", async () => {
    ...
    const acc = createAccount(db, { id: "acc_aes2", label: "a1", credit_type: "payg", api_key: "mm_test" });
    upsertModel(db, { name: "MiniMax-M2.7", upstream_model: "MiniMax-M2.7" });
    setSetting(db, "transport", { relay: null, proxy: null });
    clearCache();

    vi.spyOn(globalThis, "fetch").mockImplementation(async () => new Response(
      JSON.stringify({ base_resp: { status_code: 1002 }, message: "rate" }),
