...
import { parseArgs } from '../../scripts/add-account.cliArgs.js';
...
describe('parseArgs', () => {
  ...
  it('kiro: parses optional region, client-id, client-secret, profile-arn', () => {
    const args = parseArgs([
      ...
      '--profile-arn', 'arn:aws:codewhisperer:us-west-2:123:profile/XYZ',
    ]);
  ...
  it('codebuddy: parses optional --label and --base-url', () => {
    const args = parseArgs([
      ...
      '--api-key', 'cb_secret_xxx',
      '--label', 'my-codebuddy',
      '--base-url', 'https://custom.codebuddy.ai',
    ]);
