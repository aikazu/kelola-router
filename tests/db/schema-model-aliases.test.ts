...
describe('model_aliases schema', () => {
  it('creates model_aliases table with expected columns', () => {
    ...
    expect(names).toEqual(
      expect.arrayContaining(['alias_name', 'upstream_model', 'label', 'source', 'created_at'])
    );
  ...
  it('creates idx_model_aliases_target index', () => {
    const idx = db
      .prepare(
        "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_model_aliases_target'"
      )
      .get();
  ...
  it('FK cascades when upstream model is deleted', () => {
    db.prepare(
      `INSERT INTO models (name, upstream_model, source) VALUES ('m1', 'm1-up', 'manual')`
    ).run();
    db.prepare(
      `INSERT INTO model_aliases (alias_name, upstream_model) VALUES ('a1', 'm1-up')`
    ).run();
