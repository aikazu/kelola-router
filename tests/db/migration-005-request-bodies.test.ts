...
describe("migration 005", () => {
  ...
  it("allows inserting all new columns", () => {
    ...
    `).run(
      "test-model", "/v1/chat/completions", "openai", 200, 123, 10, 20, 30, 0.001,
      '{"messages":[]}', '{"content":"hi"}',
      '{"content-type":"application/json"}', '{"x-request-id":"abc"}',
      null
    );
    const row = db.prepare("SELECT * FROM request_logs ORDER BY id DESC LIMIT 1").get() as Record<string, unknown>;
