...
describe("requestLogs repo", () => {
  it("inserts and reads body fields", () => {
    const id = insertRequestLog(db, {
      model: "m1", endpoint: "/v1/x", format: "openai",
      prompt_tokens: 5, completion_tokens: 10, cache_creation_tokens: 0, cache_read_tokens: 0,
      total_tokens: 15, cost_usd: 0.0001, latency_ms: 100, status_code: 200, stream: false, rtk_bytes_saved: 0,
  ...
  it("stores error when set", () => {
    const id = insertRequestLog(db, {
      model: "m1", endpoint: "/v1/x", format: "openai",
      prompt_tokens: 0, completion_tokens: 0, cache_creation_tokens: 0, cache_read_tokens: 0,
      total_tokens: 0, cost_usd: 0, latency_ms: 50, status_code: 500, stream: false, rtk_bytes_saved: 0,
