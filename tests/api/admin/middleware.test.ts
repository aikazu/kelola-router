...
import { requireAdminJson, handleApiError, ApiError } from "../../../src/api/admin/middleware.js";
...
describe("requireAdminJson", () => {
  it("returns 401 JSON when no session and password is set", async () => {
    ...
    db.prepare(`INSERT INTO settings (key, value) VALUES ('admin_password', ?)`).run(JSON.stringify(hashPassword("secret123")));
    const app = new Hono();
