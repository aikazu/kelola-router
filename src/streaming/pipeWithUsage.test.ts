...
describe("pipeWithUsage", () => {
  ...
  it("extracts OpenAI usage after stream completes", async () => {
    ...
    const out = await pipeWithUsage(sseResponse(raw), "openai", (u) => { captured = u; });
    await out.text();
    expect(captured).toEqual({ prompt_tokens: 10, completion_tokens: 5, cache_creation_tokens: 0, cache_read_tokens: 0, total_tokens: 15 });
  });
  ...
  it("extracts Anthropic usage from message_delta", async () => {
    ...
    let captured: SSEUsage | null = undefined as unknown as SSEUsage | null;
    const out = await pipeWithUsage(sseResponse(raw), "anthropic", (u) => { captured = u; });
    await out.text();
    expect(captured).toEqual({ prompt_tokens: 7, completion_tokens: 3, cache_creation_tokens: 0, cache_read_tokens: 0, total_tokens: 10 });
  });
  ...
  it("returns upstream unchanged when body is null and reports null usage", async () => {
    ...
    let captured: SSEUsage | null = undefined as unknown as SSEUsage | null;
    const out = await pipeWithUsage(r, "openai", (u) => { captured = u; });
