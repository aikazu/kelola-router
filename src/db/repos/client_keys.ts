...
export function getClientKey(db: Database.Database, id: number): ClientKey | null {
  return db.prepare(`SELECT * FROM client_keys WHERE id = ?`).get(id) as ClientKey | null;
}

export function getClientKeyByKey(db: Database.Database, key: string): ClientKey | null {
  return db.prepare(`SELECT * FROM client_keys WHERE key = ? AND enabled = 1`).get(key) as ClientKey | null;
}
