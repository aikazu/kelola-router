...
export function renderUsage(db: Database.Database, clientKeyId?: number): string {
  ...
  const body = `
    ...
    <table>
      ...
      ${logs.map((l) => `<tr><td>${l.created_at}</td><td>${l.client_key_id ?? ""}</td><td>${l.model}</td><td>${escapeHtml(acctById.get(l.account_id ?? "")?.label ?? l.account_id ?? "")}</td><td>${l.total_tokens}</td><td>$${l.cost_usd.toFixed(4)}</td><td>${l.status_code}</td><td>${l.latency_ms}ms</td></tr>`).join("")}
    </table>
  `;
