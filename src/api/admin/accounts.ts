...
import {
  listAccounts, createAccount, enableAccount, disableAccount, deleteAccount,
...
accountRoutes.post("/", (c) => {
  try {
    return c.req.json().then((body: { label?: string; credit_type?: string; api_key?: string; base_url?: string }) => {
      ...
      const acc = createAccount(db, {
        id: ulid(), label: body.label, credit_type: body.credit_type as "payg" | "token-plan", api_key: body.api_key,
