...
overviewRoutes.get("/overview", (c) => {
  try {
    ...
    return c.json({
      ...
      recent: recent.map(r => ({
        id: r.id, createdAt: r.created_at, model: r.model, statusCode: r.status_code,
        cost: r.cost_usd, latencyMs: r.latency_ms, clientKeyId: r.client_key_id, accountId: r.account_id,
      })),
    });
